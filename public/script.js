...
const setUpDocument = async n => {
  ...
  const work = async (image, pageNum) => {
    ...
    worker.setParameters({ 'user_defined_dpi': (n * 80).toString() });
...
const runOCR = async ocrJson => {
  ...
  try {
    ...
    /*await PDFNet.OCRModule.applyOCRJsonToPDF(doc, JSON.stringify(ocrJson))
...
const initializeWebViewer = async _ => {
  const makeWebViewerInstance = async _ => WebViewer(
    {
      path: './webviewer/lib',
      fullAPI: true,
    },
  ...
  const instance = await makeWebViewerInstance();
  ...
  instance.docViewer.on('documentLoaded', _ => webViewerDoc = instance.docViewer.getDocument());
